...
public:
    ...
    void identify(){
        for(int i=0; i<foragers.size(); i++){foragers[i]->id = i;}
        for(int i=0; i<sentinels.size(); i++){sentinels[i]->id = i+foragers.size();}
        for(int i=0; i<threats.size(); i++){threats[i]->id = i+foragers.size()+sentinels.size();}
    }
        ...
        for (auto& forager : foragers) {
            ...
            for (size_t s = 0; s < sentinels.size(); ++s) {
                if(sentinels[s]->is_dominant && (rand() / float(RAND_MAX)) < DOMINANT_RANGE){
                    perceived[s] = *max_element(sentinels[s]->threat_readings.begin(),sentinels[s]->threat_readings.end()) * environmentalNoise(forager->getPosition(), sentinels[s]->getPosition());
                    sentinels[s]->last_action = *max_element(sentinels[s]->threat_readings.begin(),sentinels[s]->threat_readings.end());
                }
                else {perceived[s] = sentinels[s]->chooseAlertIntensity() * environmentalNoise(forager->getPosition(), sentinels[s]->getPosition());}
            }
    ...
    /*
    verificar o sentinela mais próximo
    Se estiver no range, ataca
    se a ação do sentinela for "fugir" ele escapa
    */
   vector<int> threatAttack(){
    int rewardForager;
    int rewardSentinel;
        for(auto& threat : threats){
            for(auto& forager : foragers){
                if(distanceBetween(threat->getPosition(), forager->getPosition())<= ATTACK_RANGE){
                    if(forager->last_action == 1){
                        rewardForager = 5;
                        break;
                    } else{
                        forager->die();
        ...
        return {rewardForager, rewardSentinel};
    }
    ...
    void executeActions() {
        ...
        for (size_t i = 0; i < foragers.size(); ++i) {
            ...
            float reward = 
            calculateRewardForager(action, std::get<2>(current_state), forage_[i])
            + threatAttack()[0];
        ...
        for (auto& sentinel : sentinels) {
            ...
            float reward = 
            calculateRewardSentinel(action, std::get<2>(current_state), forage_)
            + threatAttack()[1];
